...
import ImagenProductoDefault from "../imagenes/default-product-pic.jpg";
...
const AdminAddProduct = (props) => {
  ...
  useEffect(() => {
    if (!isEdit)
      setState({ ...state, supplier_id: 1, category_id: 1});
    ...
    if(!stateImage.image){
      if (isEdit) {
        try {
          ...
          console.log("Posiblemente no existe una imágen para este producto! (lo cual sería muy raro)");
          console.log({ error });
        }
      } else {
  ...
  const loadSubcategories = async (category_id) => {
    ...
    try {
      ...
      console.log("Response de la wea");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if(stateImage.image) {
        ...
        await openAlert("Se debe seleccionar la imágen del producto");
        return;
      }
